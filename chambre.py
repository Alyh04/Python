...
class Fenetre_chambre:
    def __init__(self, root):
        ...
        #=========Titre ambony eo iny===========
        lbl_title =Label(self.root,text="RESERVER UNE CHAMBRE", font=("times new roman",18,"bold"),bg="black",fg="gold",bd=4,relief=RIDGE)
        ...
        lblimag.place(x=5, y=2, width=100, height=40)
                ...
                #=========label ===========
        labelframeleft=LabelFrame(self.root,relief=RIDGE,text="Information de la chambre",font=("times new roman",12,"bold"),padx=2,)
        ...
        btnfetchData=Button(labelframeleft,command=self.Fetch_Contact,text="Afficher",font=("arial",8,"bold"),bg="gray",fg="white",width=9)
        ...
        # sorti
        check_out=Label(labelframeleft,text="Date de sorti :",font=("arial",10,"bold"),padx=2,pady=6)
        check_out.grid(row=2,column=0,sticky=W)
        txtcheck_out=ttk.Entry(labelframeleft,textvariable=self.var_Date_Sortie,width=29,font=("arial",10,"bold"))
        ...
        combo_RoomType=ttk.Combobox(labelframeleft,font=("arial",10,"bold"),textvariable=self.var_Chambre,width=27,state="readonly")
        combo_RoomType["value"]=id
        combo_RoomType.current(0)
        combo_RoomType.grid(row=3,column=1)

        # Chambre_Utiliser
        lblRoomChambre_Utiliser=Label(labelframeleft,text="Chambre Utiliser :",font=("arial",10,"bold"),padx=2,pady=6)
        ...
        combo_RoomNo=ttk.Combobox(labelframeleft,font=("arial",10,"bold"),textvariable=self.var_Chambre_Utiliser,width=27,state="readonly")
        ...
        combo_diner=ttk.Combobox(labelframeleft,font=("arial",10,"bold"),textvariable=self.var_Diner,width=27,state="readonly")
        combo_diner["value"]=("Pizza","Tacos","spaghetti","Sushi","Soupe","Composée","Mine-sao","Crevette","Viande_Haché","viande_Fritte","Poulet_Routi","Poulet_pané","Poulet_Frit","Pomme_Fritte","Pomme_de_Terre_Soupe","Soupe_Chinoise","Soupe_Zaré","Spaghetti_Bolloniaise")
        ...
        NombreDeJour=ttk.Entry(labelframeleft,width=29,textvariable=self.var_NombreDeJour,font=("arial",10,"bold"))
        NombreDeJour.grid(row=6,column=1)


        # pour boire
        lbltax=Label(labelframeleft,text="Pour boire :",font=("arial",10,"bold"),padx=2,pady=6)
        lbltax.grid(row=7,column=0,sticky=W)
        txttax=ttk.Entry(labelframeleft,width=29,textvariable=self.var_PourBoire,font=("arial",10,"bold"))
        txttax.grid(row=7,column=1)


        # Montant
        lblTotal=Label(labelframeleft,text="Montant Partielle :",font=("arial",10,"bold"),padx=2,pady=6)
        lblTotal.grid(row=8,column=0,sticky=W)
        txtTotal=ttk.Entry(labelframeleft,width=29,textvariable=self.var_Montant,font=("arial",10,"bold"))
        txtTotal.grid(row=8,column=1)


        # total
        lbltotal=Label(labelframeleft,text="Total du prix :",font=("arial",10,"bold"),padx=2,pady=6)
        lbltotal.grid(row=9,column=0,sticky=W)
        txttotal=ttk.Entry(labelframeleft,width=29,textvariable=self.var_Total,font=("arial",10,"bold"))
        txttotal.grid(row=9,column=1)


        # Addition
        btnBill=Button(labelframeleft,command=self.total,text="Addition",font=("arial",12,"bold"),bg="black",fg="gold",width=9)
        ...
        btnReset=Button(btn_frame,text="Vider",command=self.reset,font=("arial",12,"bold"),bg="black",fg="white",width=9)
        btnReset.grid(row=0,column=3,padx=1)


        # photo a droite
        img3 = Image.open(r"E:\Lorniot Marcel (leçons) L2\Python\Projet\ProjetPython\GestionHotel\image\VueDEhors.jpg")
        ...
        lblimag.place(x=750, y=55, width=450, height=250)
         ...
         # ======== Table amin'ny CRUD==========
         #=========label ===========
        Table_Frame=LabelFrame(self.root,relief=RIDGE,text="Table des Informations des Chambres & Recherche",font=("times new roman",12,"bold"),padx=2,)
        ...
        btnSearch=Button(Table_Frame,command=self.search,text="Recherche",font=("arial",10,"bold"),bg="green",fg="white",width=9)
        ...
        self.room_table=ttk.Treeview(details_table,columns=("Contact","Date_Entree","Date_Sortie","Chambre","Chambre_Utiliser","Diner","NombreDeJour","PourBoire","Montant","Total"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.room_table.heading("Date_Sortie",text="Date_Sortie")
        ...
        self.room_table.heading("PourBoire",text="PourBoire")
        self.room_table.heading("Montant",text="Montant")
        ...
        self.room_table.column("Date_Sortie",width=100)
        ...
        self.room_table.column("PourBoire",width=100)
        self.room_table.column("Montant",width=100)
    ...
    def add_data(self):
            if self.var_contact.get() == "" or self.var_Date_Entree.get() == "":
                ...
                try:
                    conn=mysql.connector.connect(host="localhost",username="root",password="",database="hotel")
                    my_cursor=conn.cursor()
                    my_cursor.execute("INSERT INTO chambre VALUES(%s,%s,%s,%s,%s,%s,%s)", (
                        self.var_contact.get(),
                        self.var_Date_Entree.get(),
                        self.var_Date_Sortie.get(),
                    ...
                    self.fetch_data()
                    conn.close()
                    messagebox.showinfo("Success","L'Addittion ajouté avec succès",parent=self.root)
    ...
    def get_cursor(self,event=""):
        ...
        self.var_Date_Entree.set(row[1])
        self.var_Date_Sortie.set(row[2])
    ...
    def update(self):
        if self.var_contact.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="", database="hotel")
                my_cursor = conn.cursor()
                my_cursor.execute("UPDATE chambre SET Date_Entree=%s, Date_Sortie=%s, Chambre=%s, Chambre_Utiliser=%s, Diner=%s, NombreDeJour=%s WHERE Contact=%s", (
                        self.var_Date_Entree.get(),
                        self.var_Date_Sortie.get(),
                        self.var_Chambre.get(),
                        self.var_Chambre_Utiliser.get(),
        ...
        # supprimer toy lahy 
    def mDelete(self):
        mDelete=messagebox.askyesno("Systeme Gestion d'Hotel","Voulez-vous supprimer ce client ?",parent=self.root)
        if mDelete > 0 :
            conn=mysql.connector.connect(host="localhost",username="root",password="",database="hotel")
            my_cursor=conn.cursor()
    ...
    def search(self):
        ...
        my_cursor.execute("SELECT * FROM chambre WHERE " + str(self.search_var.get()) + " LIKE '%" + str(self.txt_search.get()) + "%'")
        rows = my_cursor.fetchall()
        if len(rows) != 0:
            self.room_table.delete(*self.room_table.get_children())
            for i in rows:
    ...
    def total(self):
        ...
        # elif (self.var_Diner.get()=="Tacos" and self.var_Chambre.get()=="Solitaire") :
        #     q1=float(5000)
        #     q2=float(10000)
        #     q3=float(self.var_NombreDeJour.get())
        #     q4=float(q1+q2)
        ...
        # elif (self.var_Diner.get()=="Tacos" and self.var_Chambre.get()=="Double") :
        #     q1=float(5000)
        #     q2=float(15000)
        #     q3=float(self.var_NombreDeJour.get())
        #     q4=float(q1+q2)
        ...
        # elif (self.var_Diner.get()=="Tacos" and self.var_Chambre.get()=="Luxe") :
        #     q1=float(5000)
        #     q2=float(20000)
        #     q3=float(self.var_NombreDeJour.get())
        #     q4=float(q1+q2)
