...
class Fenetre_Client:
    def __init__(self, root):
        ...
        self.var_addresse=StringVar()
         ...
         #=========Titre ambony eo iny===========
        lbl_title =Label(self.root,text="AJOUTER UN CLIENT", font=("times new roman",18,"bold"),bg="black",fg="gold",bd=4,relief=RIDGE)
        ...
        # code postal
        label_codepostal=Label(labelframeleft,text="Code Postal :",font=("arial",10,"bold"),padx=2,pady=6)
        label_codepostal.grid(row=4,column=0,sticky=W)
        entry_codepostal=ttk.Entry(labelframeleft,textvariable=self.var_postal,width=29,font=("arial",10,"bold"))
        entry_codepostal.grid(row=4,column=1)

        # numero
        label_number=Label(labelframeleft,text="Numéro :",font=("arial",10,"bold"),padx=2,pady=6)
        ...
        combo_id["value"]=("Cart d'Identité National","Pass-port")
        combo_id.current(0)
        combo_id.grid(row=8,column=1)

        # id
        label_id=Label(labelframeleft,text="Id :",font=("arial",10,"bold"),padx=2,pady=6)
        ...
        # Addresse
        label_address=Label(labelframeleft,text="Addresse :",font=("arial",10,"bold"),padx=2,pady=6)
        label_address.grid(row=10,column=0,sticky=W)
        entry_address=ttk.Entry(labelframeleft,textvariable=self.var_addresse,width=29,font=("arial",10,"bold"))
        ...
        btnUpdate=Button(btn_frame,text="Modifier",command=self.update,font=("arial",11,"bold"),bg="purple",fg="white",width=9)
        ...
        self.Cust_Details_Table=ttk.Treeview(details_table,columns=("Reference","Nom","Prenom","Genre","Postal","Numero","Email","Nationalite","Preuve","Id","Addresse"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.Cust_Details_Table.heading("Postal",text="Postal")
        ...
        self.Cust_Details_Table.heading("Addresse",text="Addresse")
        ...
        self.Cust_Details_Table.column("Addresse",width=100)
    ...
    def add_data(self):
        if self.var_numero.get() == "" or self.var_prenom.get() == "":
            ...
            try:
                ...
                my_cursor.execute("INSERT INTO client VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
                    self.var_ref.get(),
                    self.var_nom.get(),
                    self.var_prenom.get(),
                    self.var_genre.get(),
    ...
    def get_cursor(self,event=""):
        ...
        self.var_numero.set(row[5]),
        self.var_email.set(row[6]),
        self.var_nationalite.set(row[7]),
        self.var_preuve.set(row[8]),
        self.var_id.set(row[9])
        self.var_addresse.set(row[10])
    ...
    def update(self):
        if self.var_numero.get()=="":
            ...
            try:
                conn=mysql.connector.connect(host="localhost",username="root",password="",database="hotel")
                my_cursor=conn.cursor()
                my_cursor.execute("UPDATE client SET Nom=%s,Prenom=%s,Genre=%s,Postal=%s,Numero=%s,Email=%s,Nationalite=%s,Preuve=%s,Id=%s,Addresse=%s WHERE Reference=%s", (
    ...
    def reset(self):
        ...
        # self.var_preuve.set(""),
        self.var_id.set("")
        self.var_addresse.set("")

        # toy mampiova chiffre reference io
        x=random.randint(1000,9999)
        self.var_ref.set(str(x))
